...
namespace S22.Imap
{
    ...
    internal static class MessageBuilder
    {
        ...
        /// <param name="encoding">An predefined encoding which should be used for converting the string to the mailmessage.</param>
        ...
        internal static MailMessage FromMIME822(string text, Encoding encoding = null)
        {
            ...
            foreach (MIMEPart p in parts)
                m.AddBodypart(BodypartFromMIME(p), p.body, encoding);
            return m;
        }
        ...
        /// <param name="predefinedEncoding">The encoding which should be used for adding the part.</param>
        internal static void AddBodypart(this MailMessage message, Bodypart part, string content, Encoding predefinedEncoding = null)
        {
            Encoding encoding;
            if (predefinedEncoding == null)
            {
                encoding = part.Parameters.ContainsKey("Charset") ? Util.GetEncoding(part.Parameters["Charset"]) : Encoding.ASCII;
            }
            else
            {
                encoding = part.Parameters.ContainsKey("Charset") ? Util.GetEncoding(part.Parameters["Charset"]) : predefinedEncoding;
            }
            ...
            try
            {
                switch (part.Encoding)
                {
                    ...
                    default:
                        ...
                        {
                            bytes = predefinedEncoding.GetBytes(content);
                        }
                        break;
                }
            }
                ...
                part.Disposition.Type != ContentDispositionType.Attachment)
            {
                ...
                {
                    message.Body = predefinedEncoding.GetString(bytes);
                }
