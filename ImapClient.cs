...
namespace S22.Imap
{
    ...
    public class ImapClient : IImapClient
    {
        ...
        /// <param name="encoding">Sets a Encoding which should be used for fetching the mails form the given mailprovider</param>
        ...
        public ImapClient(string hostname, int port = 143, bool ssl = false,
            RemoteCertificateValidationCallback validate = null, Encoding encoding = null)
        {
            _encoding = encoding;
                ...
                lock (readLock)
                {
                    while (true)
                    {
                        ...
                        if (b == Newline)
                        {
                            ...
                            if (_encoding == null)
                            {
                                s = Encoding.ASCII.GetString(mem.ToArray());
                            }
                            else
                            {
                                s = _encoding.GetString(mem.ToArray());
                            }
                            if (resolveLiterals)
                            {
        ...
        string GetData(int byteCount)
        {
            ...
            using (var mem = new MemoryStream())
            {
                ...
                string s = Encoding.GetEncoding(1252).GetString(mem.ToArray());
                ts.TraceInformation("S -> " + s);
                return s;
            }
        }
            ...
            bool seen = true, string mailbox = null)
        {
            ...
            switch (options)
            {
                ...
                case FetchOptions.NoAttachments:
                    return GetMessage(uid, p =>
                    {
                        return p.Disposition.Type !=
                ...
                case FetchOptions.TextOnly:
                    return GetMessage(uid, p => { return p.Type == ContentType.Text; },
                        seen, mailbox);
                default:
                    return MessageBuilder.FromMIME822(GetMessageData(uid, seen, mailbox), _encoding);
            }
        }
                ...
                try
                {
                    ...
                    foreach (Bodypart part in parts)
                    {
                        ...
                        if (callback(part) == true)
                        {
                            ...
                            message.AddBodypart(part, content, _encoding);
                        }
                    }
                }
                catch (FormatException)
